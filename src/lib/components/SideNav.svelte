...
<div class="hidden md:flex flex-col w-64 bg-base-200 min-h-screen fixed left-0 top-0 border-r border-base-300 z-50">
  <div class="p-6 flex items-center gap-3">
    <svg class="w-8 h-8 text-primary" viewBox="0 0 100 114.42231082293117">
    ...
    <h1 class="text-2xl font-bold text-primary tracking-wide">MIGRU</h1>
  </div>
  ...
  <nav class="flex-1 px-4 space-y-2">
    {#each navItems as item}
      <a
        href={item.href}
        class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 hover:bg-base-300
        {($page.url.pathname === item.href) ? 'bg-primary text-primary-content shadow-lg' : 'text-base-content/70'}"
      >
  ...
  <div class="p-4 border-t border-base-300">
    <a 
      href="/settings" 
      class="flex items-center gap-3 px-4 py-3 w-full rounded-xl transition-all duration-200 hover:bg-base-300
      {($page.url.pathname === '/settings') ? 'bg-base-300 text-base-content font-bold' : 'text-base-content/70'}"
    >
